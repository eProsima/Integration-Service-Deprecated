...
public:
	...
	//!Initialize
	bool init(int transport, eprosima::fastrtps::ReliabilityQosPolicyKind kind, int samples,
        int wait_time, const std::string& topicName, int domain, eprosima::fastrtps::TopicDataType* type,
		const std::string& name, unsigned short port = 0);
	//!Publish a sample
	bool publish();
