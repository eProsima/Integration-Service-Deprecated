...
public:
    ...
    void block(std::function<bool()> checker, uint32_t maxWait = 0)
    {
        std::unique_lock<std::mutex> lock(mutex_);
        if (maxWait > 0)
        {
            cv_.wait_for(lock, std::chrono::milliseconds(maxWait), checker);
        }
        else
        {
    ...
    size_t block_for_at_least(size_t at_least, uint32_t maxWait = 0)
    {
